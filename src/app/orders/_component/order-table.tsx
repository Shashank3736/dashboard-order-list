...
export const OrderTable = () => {
  ...
  const allCurrentSelected =
    ...
    currentOrderIds.every((id) => selectedOrders.has(id));
  const someCurrentSelected = currentOrderIds.some((id) =>
    selectedOrders.has(id)
  );
  ...
  return (
    <div className="w-full">
      <div className="bg-card flex items-center justify-between rounded-lg px-4 py-2">
          ...
          <tbody>
            {currentOrders.map((order) => (
              <tr
                key={order.id}
                className={cn(
                  'hover:bg-card border-b transition-colors',
      ...
      {selectedOrders.size > 0 && (
        <div className="mt-4 rounded-lg border border-blue-200 bg-blue-50 px-4 py-2 dark:border-blue-800 dark:bg-blue-950/20">
      ...
      {/* Pagination Controls */}
      <div className="mt-6 flex flex-col items-center justify-between gap-2 px-4 md:flex-row">
